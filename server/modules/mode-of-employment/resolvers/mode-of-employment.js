...
const getModeOfEmployments = async (_, args, { me, tenantid })  => new Promise(async (resolve, reject) => {
  const param = me?.user?.username === 'gonngod' ? paramHandler({...args.query}): paramHandler({...args.query, tenantid})
  try {
    await ModeOfEmployment.find(param)
...
module.exports = getModeOfEmployments;
